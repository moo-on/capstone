...
df = df[df.index <= datetime.datetime(2021,4,27)]
...
for i in range(len(df.close)-1):
    if df.buy.values[i] == 0:
        if df.PB.values[i] > 0.7 and df.MFI10.values[i] > 55 and df.high.values[i] > df.target.values[i]:       #%b가 0.7보다 크고 10일 기준 MFI가 70보다 크고 변동성 돌파를 만족하면
            df.buyTarget.values[i] = df.close.values[i]
            df.buy.values[i+1] = 1
        else:
            df.buyTarget.values[i] = 0
    else:
        if df.PB.values[i] < 0.3 and df.MFI10.values[i] < 45:
