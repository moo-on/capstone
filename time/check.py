...
#df = df[df1.index <= datetime.datetime(2018,1,14)]

for k in np.arange(6,10,0.5):
    df['MA20'] = df['종가'].rolling(window=20).mean().shift(1)
    df['stddev'] = df['종가'].rolling(window=20).std().shift(1)
    ...
    df['buy_k'] = df['MA20'] + (df['stddev'] * 2 * (k - 5) * 0.2)
    df['sell_k'] = df['MA20'] - (df['stddev'] * 2 * (k - 5) * 0.2)
